...
description: Enforce the HelloDev four-document contract in a repository — CODINGSTYLE.md, DESIGN.md, LOG.md, and docs/adr/. Use at the start of work in any repo, when any of those files is missing, or when the user says init project docs, set up conventions, bootstrap governance, project standards, or /hd-init. Detects the tech stack from the existing code instead of asking when the repo already has code, and interviews the user when it does not.
---

# Project Governance

Every repository that uses HelloDev carries four artifacts. They are the durable memory of the project — the part a fresh agent session cannot reconstruct from the code alone.
...
> `DESIGN.md` and `docs/adr/` are missing. Run `/hd-init` to set them up.

Do not silently proceed without `CODINGSTYLE.md`. If it is absent and the user asks for implementation work, say so and offer `/hd-init` first. If they decline, infer conventions from sibling files and note in your reply that conventions were inferred, not governed.
