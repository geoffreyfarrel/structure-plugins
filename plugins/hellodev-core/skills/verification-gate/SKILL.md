...
description: Detect the repository toolchain and run the correct format, lint, typecheck, build, and test commands before declaring work complete. Use before committing, before opening a PR or MR, and whenever the user says verify, check, run the checks, is it working, is it done, or /hd-check. Never report work as finished on an unverified change.
---
